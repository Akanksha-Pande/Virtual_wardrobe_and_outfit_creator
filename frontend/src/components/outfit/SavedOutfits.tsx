...
export function SavedOutfits({ onNavigate }: SavedOutfitsProps) {
  ...
  const handleDeleteOutfit = async (id: string) => {
    const res = await fetch(`http://localhost:8080/api/outfits/${id}`, {
      method: 'DELETE',
    });
        ...
        {localOutfits.length === 0 ? (
          ...
          localOutfits.map((outfit) => {
            ...
            return (
              ...
              >
                {/* Outfit stacked preview */}
                <div className="relative bg-gray-50 flex flex-col items-center justify-center p-4 h-50 w-30 rounded-t-2xl">
                  <div className="absolute inset-0 bg-gradient-to-b from-gray-100 to-gray-200 rounded-2xl" />
                  ...
                  <div className="relative z-10 flex flex-col items-center justify-center space-y-2">
                    {sortedItems.map((item) => (
                      <img
                        ...
                        src={item.imagePath}
                        alt={item.name}
                        className="w-20 h-20 object-contain rounded-lg shadow-sm border border-gray-100"
                      />
                    ))}
                  </div>
                ...
                {/* Outfit Info */}
                <div className="p-4">
                  <h3 className="font-semibold text-gray-900 text-sm truncate">{outfit.name}</h3>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-xl w-full max-w-sm text-center">
            ...
            <div className="flex justify-center space-x-3">
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg bg-red-500 text-white hover:bg-gray-600 transition"
              >
                Delete
              </button>
            </div>
      ...
      {selectedOutfit && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl relative">
            ...
            <div className="p-4 w-25 h-25 flex flex-col items-center">
              ...
              {/* stacked mannequin-style preview */}
              <div className="relative bg-gradient-to-b from-gray-100 to-gray-200 rounded-3xl p-6 shadow-inner border w-64 h-[350px] flex flex-col items-center justify-center">
                  ...
                  {selectedOutfit.items
                    ...
                    .map((item) => (
                      <img
                        ...
                        src={item.imagePath}
                        alt={item.name}
                        className="w-32 h-34 object-contain drop-shadow-md"
                      />
                    ))}
                </div>
              </div>
